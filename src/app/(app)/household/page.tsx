...
import { Users, UserPlus, Crown, Shield, User, Trash2, Mail, Home, Copy, Check, Pencil } from 'lucide-react'
...
export default function HouseholdPage() {
  ...
  async function removeMember(userId: string) {
    if (userId === currentUserId) { toast('Você não pode se remover!', 'warning'); return }
    const supabase = createClient()
    if (!household?.id) return
    await supabase.from('household_members').delete().eq('household_id', household.id).eq('user_id', userId)
    toast('Membro removido', 'info'); load()
  }

  async function renameHousehold() {
    if (!newName.trim() || !household) return
    const supabase = createClient()
    await supabase.from('households').update({ name: newName.trim(), updated_at: new Date().toISOString() }).eq('id', household.id)
  ...
  if (!household) return (
    <div className="flex flex-col items-center justify-center h-64 gap-4 text-center px-6">
      ...
      <Button onClick={() => window.location.href = '/onboarding'}>Criar lar</Button>
    </div>
  )
  ...
  return (
    <div className="px-4 py-4 max-w-2xl mx-auto lg:px-6 lg:py-6 space-y-4">
                ...
                <li key={m.id} className={`flex items-center gap-3 px-5 py-3.5 ${i < members.length - 1 ? 'border-b border-[#1a2e1a]' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-green-200 truncate">
                      {m.profiles?.full_name || 'Membro'} {isMe && <span className="text-xs text-green-700">(você)</span>}
                    </p>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <Icon size={11} className={m.role === 'owner' ? 'text-yellow-500' : m.role === 'admin' ? 'text-blue-400' : 'text-green-700'} />
                      <span className="text-[10px] text-green-700">{ROLE_LABELS[m.role]}</span>
                    </div>
                  </div>
                  {!isMe && currentRole === 'owner' && (
                    <button onClick={() => removeMember(m.user_id)} className="p-1.5 rounded-lg hover:bg-red-900/20 text-green-700 hover:text-red-400 transition-colors">
                      <Trash2 size={14} />
                    </button>
                  )}
                </li>
      ...
      <Modal open={showRenameModal} onClose={() => setShowRenameModal(false)} title="Renomear lar" size="sm">
        <div className="p-5 flex flex-col gap-4">
          <Input label="Novo nome" value={newName} onChange={e => setNewName(e.target.value)} icon={<Home size={16} />} onKeyDown={e => e.key === 'Enter' && renameHousehold()} />
          <div className="flex gap-3">
            <Button variant="secondary" onClick={() => setShowRenameModal(false)} className="flex-1">Cancelar</Button>
            <Button onClick={renameHousehold} className="flex-1">Salvar</Button>
          </div>
        </div>
      </Modal>
    </div>
